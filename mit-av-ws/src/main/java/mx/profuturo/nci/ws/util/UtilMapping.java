...
import mx.profuturo.nci.ws.beans.CifrasGeneralesPorTipoBean;
...
import mx.profuturo.nci.ws.util.enums.TransaccionMappingEnum;
...
public final class UtilMapping {
	...
	public static ClaveSolicitudDomiciliacionBean mappingClaveSolicitudDomiciliacionVO(SolicitudVO vo) throws MitBusinessException {
		
		ClaveSolicitudDomiciliacionBean bean = null;

		try {
			if (vo != null) {
				bean = new ClaveSolicitudDomiciliacionBean();
				mapVoToBean(bean, vo);
			}
...
//			}
			...
			if (conciliacion.getDiversificaciones() != null) {
				...
				for (DiversificacionConciliacionVO diversificacion : conciliacion.getDiversificaciones()) {
					...
					if (diversificacion.getFondoApovol() != null) {
						GenericoCatalogoBean fondoAportacion = new GenericoCatalogoBean();
						mapVoToBean(fondoAportacion, diversificacion.getFondoApovol());
						diversificacionConciliacionBean.setFondoApovol(fondoAportacion);
					}
...
//		Se agregan al mapper, clase:  mx.profuturo.nci.ws.util.BeanMap.java
//			if(conciliacion.getNombreOrdenante() != null){
			...
			if (conciliacion.getDiversificaciones() != null) {
				...
				for (DiversificacionConciliacionBean diversificacion : conciliacion.getDiversificaciones()) {
					...
					if (diversificacion.getFondoApovol() != null) {
						...
						mapBeanToVO(diversificacion.getFondoApovol(), fondoAportacion);
						diversificacionConciliacionVO.setFondoApovol(fondoAportacion);
					}
	...
	public static DiversificacionOrdenAportacionBean mappingDiversificacionOrdenesVO(final DiversificacionOrdenesVO vo)throws MitBusinessException
	{
		...
		try
		{
			...
			if(vo != null)
			{
				...
				if(vo.getFondoApovol() != null)
				{
					GenericoCatalogoBean fondoApovolBean=new GenericoCatalogoBean();
					mapVoToBean(fondoApovolBean, vo.getFondoApovol());
					
					bean.setFondoApovol(fondoApovolBean);
	...
	public static DiversificacionOrdenesVO mappingDiversificacionOrdenAportacionBean(final DiversificacionOrdenAportacionBean bean,final String numeroOrden,final String usuario,final TransaccionMappingEnum transaccion)throws MitBusinessException
	{
		...
		try
		{
			if(bean != null)
			{
				...
				if(bean.getFondoApovol() != null)
				{
					GenericCatalogoVO fondoApovolVo=new GenericCatalogoVO();
					mapBeanToVO(bean.getFondoApovol(), fondoApovolVo);
					vo.setFondoApovol(fondoApovolVo);
				}
