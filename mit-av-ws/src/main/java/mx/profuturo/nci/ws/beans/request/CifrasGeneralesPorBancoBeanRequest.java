package mx.profuturo.nci.ws.beans.request;

public class CifrasGeneralesPorBancoBeanRequest {
	private Short idClaveBanco;
	private String folioConciliacion;

	public Short getIdClaveBanco() {
		return idClaveBanco;
	}

	public void setIdClaveBanco(Short idClaveBanco) {
		this.idClaveBanco = idClaveBanco;
	}

	public String getFolioConciliacion() {
		return folioConciliacion;
	}

	public void setFolioConciliacion(String folioConciliacion) {
		this.folioConciliacion = folioConciliacion;
	}
	
	
	
}
