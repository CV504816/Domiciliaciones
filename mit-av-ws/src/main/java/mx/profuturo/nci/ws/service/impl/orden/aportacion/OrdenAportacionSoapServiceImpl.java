package mx.profuturo.nci.ws.service.impl.orden.aportacion;
...
import mx.profuturo.nci.ws.service.orden.aportacion.IOrdenAportacionSoapService;
...
public class OrdenAportacionSoapServiceImpl  implements IOrdenAportacionSoapService
{
	...
	private static final String EXCEPTION_ATRIBUTO_FONDO_APOVOL_NULO="Atributo FondoApovol.Id es nulo";
...
//				{
					if(CollectionUtils.isNotEmpty(ordenAportacion.getDiversificaciones()))
					{
						for(DiversificacionOrdenAportacionBean diverOrdenBean : ordenAportacion.getDiversificaciones())
						{
							...
							{
								if(diverOrdenBean.getFondoApovol() != null && diverOrdenBean.getFondoApovol().getId() != null)
								{
									continue;	
								}
								else
								{
									throw new IllegalArgumentException(EXCEPTION_ATRIBUTO_FONDO_APOVOL_NULO);	
								}
							}
							else
							{
